 ...
 **/
export const usePasswordToken = async ({ token, email }) => {
  ...
  let tokenEntry = user[0].password_reset_tokens[token];
  ...
  let updated_tokens = {
    ...user[0].password_reset_tokens,
    [token]: { ...user[0].password_reset_tokens[token], used: true },
  };
