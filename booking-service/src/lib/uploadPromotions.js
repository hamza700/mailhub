...
export async function uploadPromotions(config) {
  let param1 = 1;
  let param2 = 160;
  let promotions = [];

  const res = await axios.get(
    `https://api.test.hotelbeds.com/hotel-content-api/1.0/types/promotions?fields=all&language=ENG&from=${param1}&to=${param2}`,
    config
  );
  //   param1 += 1000;
  //   param2 += 1000;
  promotions = promotions.concat(res.data.promotions);
  ...
  while (promotions.length > 0) {
    batches.push(promotions.splice(0, BATCH_SIZE));
  }
