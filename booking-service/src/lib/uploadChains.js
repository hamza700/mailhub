...
export async function uploadChains(config) {
  let param1 = 1;
  let param2 = 1000;
  let chains = [];
  ...
  for (var i = 0; i < 2; i++) {
    const res = await axios.get(
      `https://api.test.hotelbeds.com/hotel-content-api/1.0/types/chains?fields=all&language=ENG&from=${param1}&to=${param2}`,
      config
    );
      param1 += 1000;
      param2 += 1000;
    chains = chains.concat(res.data.chains);
    console.log(chains);
  }
  ...
  while (chains.length > 0) {
    batches.push(chains.splice(0, BATCH_SIZE));
  }
