...
export async function uploadSegments(config) {
  let param1 = 1;
  let param2 = 15;
  let segments = [];

  const res = await axios.get(
    `https://api.test.hotelbeds.com/hotel-content-api/1.0/types/segments?fields=all&language=ENG&from=${param1}&to=${param2}`,
    config
  );
  //   param1 += 1000;
  //   param2 += 1000;
  segments = segments.concat(res.data.segments);
  ...
  while (segments.length > 0) {
    batches.push(segments.splice(0, BATCH_SIZE));
  }
