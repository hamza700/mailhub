...
export async function uploadLanguages(config) {
  let param1 = 1;
  let param2 = 36;
  let languages = [];

  const res = await axios.get(
    `https://api.test.hotelbeds.com/hotel-content-api/1.0/types/languages?fields=all&language=ENG&from=${param1}&to=${param2}`,
    config
  );
  //   param1 += 1000;
  //   param2 += 1000;
  languages = languages.concat(res.data.languages);
  ...
  while (languages.length > 0) {
    batches.push(languages.splice(0, BATCH_SIZE));
  }
